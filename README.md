...
本项目实现了完整的八阶段序态循环架构，每个阶段对应独立的模块：
...
```
miniexp/
├── orderstate.py         # 序态循环管理器
├── env.py                # 环境定义 (GridWorld)
├── modules/
│   ├── energy.py         # 能量模块 (EnergyModule)
│   ├── signal.py         # 信号模块 (SignalModule)
│   ├── data.py           # 数据模块 (DataModule)
│   ├── information.py    # 信息模块 (InformationModule)
│   ├── knowledge.py      # 知识模块 (KnowledgeModule)
│   ├── wisdom.py         # 智慧模块 (WisdomModule)
│   ├── decision.py       # 决策模块 (DecisionModule)
│   ├── action.py         # 动作模块 (ActionModule)
│   └── attention.py      # 注意力模块 (AttentionModule)
├── agent.py              # 智能体定义 (BaseAgent, BaselineAgent, EnergyAgent)
├── metrics.py            # 评价指标工具 (MetricsRecorder)
├── main.py               # 实验驱动脚本
...
# 使用conda安装
conda create -n orderstate python=3.9
conda activate orderstate
pip install -r requirements.txt
```
...
通过比较这两种智能体的表现，我们可以观察到序态循环中生存焦虑如何影响决策过程。
